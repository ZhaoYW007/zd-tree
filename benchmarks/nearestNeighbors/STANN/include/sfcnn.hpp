...
class sfcnn_work
{
   template <typename P, unsigned int D, typename N>
   friend class sfcnn;
   ...
   */
   void
   ksearch( Point q, unsigned int k, std::vector<long unsigned int> &nn_idx,
            float eps = 0 );
   void
   ksearch( Point q, unsigned int k, std::vector<long unsigned int> &nn_idx,
            std::vector<double> &dist, float eps = 0 );
   ...
   void
   compute_bounding_box( Point q, Point &q1, Point &q2, double r );
   void
   sfcnn_work_init( int num_threads );

   void
   ksearch_common( Point q, unsigned int k, long unsigned int j, qknn &que,
                   float Eps );

   inline void
   recurse( long unsigned int s, long unsigned int n, Point q, qknn &ans,
            Point &q1, Point &q2, long unsigned int bb, long unsigned int bt );
};
...
template <typename Point, typename Ptype>
void
sfcnn_work<Point, Ptype>::sfcnn_work_init( int num_threads )
{
   num_threads = 1;
   ...
   pair_iter<typename std::vector<Point>::iterator,
             typename std::vector<long unsigned int>::iterator>
       a( points.begin(), pointers.begin() ), b( points.end(), pointers.end() );
   sort( a, b, lt );
}
...
template <typename Point, typename Ptype>
void
sfcnn_work<Point, Ptype>::recurse( long unsigned int s,  // Starting index
                                   ...
                                   long unsigned int initial_scan_upper_range )
{
   if( n < 4 )
   {
      ...
      for( long unsigned int i = 0; i < n; ++i )
      {
         if( ( s + i >= initial_scan_lower_range ) &&
            ...
            continue;
         update = ans.update( points[s + i].sqr_dist( q ), pointers[s + i] ) ||
                  update;
      }
      if( update )
         compute_bounding_box( q, bound_box_lower_corner,
                               bound_box_upper_corner, sqrt( ans.topdist() ) );
      return;
   }

   if( ( s + n / 2 >= initial_scan_lower_range ) &&
       ( s + n / 2 < initial_scan_upper_range ) )
   {
   }
   else if( ans.update( points[s + n / 2].sqr_dist( q ), pointers[s + n / 2] ) )
      compute_bounding_box( q, bound_box_lower_corner, bound_box_upper_corner,
                            sqrt( ans.topdist() ) );
   ...
   if( lt( q, points[s + n / 2] ) )
   {
      ...
      recurse( s, n / 2, q, ans, bound_box_lower_corner, bound_box_upper_corner,
               initial_scan_lower_range, initial_scan_upper_range );
      if( lt( points[s + n / 2], bound_box_upper_corner ) )
         // search_queue.push(pair<int, int>(s+n/2+1, n-n/2-1));
         recurse( s + n / 2 + 1, n - n / 2 - 1, q, ans, bound_box_lower_corner,
                  bound_box_upper_corner, initial_scan_lower_range,
   ...
   else
   {
      ...
      if( lt( bound_box_lower_corner, points[s + n / 2] ) )
         // search_queue.push(pair<int, int>(s, n/2));
         recurse( s, n / 2, q, ans, bound_box_lower_corner,
                  bound_box_upper_corner, initial_scan_lower_range,
                  initial_scan_upper_range );
   }
}
...
template <typename Point, typename Ptype>
void
sfcnn_work<Point, Ptype>::compute_bounding_box( Point q, Point &q1, Point &q2,
                                                double R )
{
...
template <typename Point, typename Ptype>
void
sfcnn_work<Point, Ptype>::ksearch_common( Point q, unsigned int k,
                                          ...
                                          qknn &que, float Eps )
{
   ...
   for( long unsigned int i = query_point_index; i < initial_scan_upper_range;
        ++i )
   {
   ...
   recurse( 0, points.size(), q, que, bound_box_lower_corner,
            bound_box_upper_corner, query_point_index,
            initial_scan_upper_range );
   // cout << "Bb1: " << bound_box_lower_corner << endl;
   // cout << "Bb2: " << bound_box_upper_corner << endl;
}
...
template <typename Point, typename Ptype>
void
sfcnn_work<Point, Ptype>::ksearch( Point q, unsigned int k,
...
class sfcnn : public nnBase<Point, Dim, NumType>
{
   ...
   */
   ...
   /*!
     ...
     \param num_threads Currently unused.  (multiple processor construction
     soon!)
   */
  ...
  private:
   ...
   void
   sfcnn_init( Point *points, long int N, int num_threads = 1 )
   {
      NN.points.resize( N );
      ...
      {
         NN.pointers[i] = ( i );
         for( unsigned int j = 0; j < Dim; ++j ) NN.points[i][j] = points[i][j];
      }
...
class sfcnn<Point, Dim, float> : public nnBase<Point, Dim, float>
{
  public:
            ...
            float eps = 0 )
   {
      ...
      for( unsigned int i = 0; i < Dim; ++i ) Q[i] = q[i];
      NN.ksearch( Q, k, nn_idx, eps );
   };
   void
   ksearch( Point q, unsigned int k, std::vector<long unsigned int> &nn_idx,
